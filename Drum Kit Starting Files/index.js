// document.querySelectorAll('button')[0].addEventListener('click',function(){});  //passing a function as a input as an response,,putting parentheses will trigger it as soon as the page is loaded

// document.querySelectorAll('button')[1].addEventListener('click',function(){});

// document.querySelectorAll('button')[2].addEventListener('click',function(){});

// document.querySelectorAll('button')[3].addEventListener('click',function(){});

// document.querySelectorAll('button')[4].addEventListener('click',function(){});

// document.querySelectorAll('button')[5].addEventListener('click',function(){});

var num=document.querySelectorAll('.drum').length;
for(var i=0;i<num;i++)
{
    // takes the input from button press
    document.querySelectorAll('button')[i].addEventListener('click',function(){

       //using switch statement to change the audio

       var buttonin=this.innerHTML;
        playSound(buttonin);
    });
}

//    keyboard event listener
   document.addEventListener("keypress",function(event){
       playSound(event.key);
   })

       function playSound(key){
        switch (key) {
           case "w":
            var audio=new Audio("sounds/crash.mp3");
            audio.play();
               break;
           
            case "a":  var audio=new Audio("sounds/kick-bass.mp3");
            audio.play();
            break;

            case "s":  var audio=new Audio("sounds/snare.mp3");
            audio.play();
            break;

            case "d":  var audio=new Audio("sounds/tom-1.mp3");
            audio.play();
            break;

            case "j":  var audio=new Audio("sounds/tom-2.mp3");
            audio.play();
            break;

            case "k":  var audio=new Audio("sounds/tom-3.mp3");
            audio.play();
            break;

            case "l":  var audio=new Audio("sounds/tom-4.mp3");
            audio.play();
            break;
           default:
               break;
       }
    }






        // var audio=new Audio("sounds/tom-1.mp3");
        // audio.play();
    

